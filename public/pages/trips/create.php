...
if($fromMaster){
  try {
    ...
    $masterMap = [];
    foreach($masterRows as $mr){ $masterMap[$mr['EmpNo']] = $mr; }
    foreach($employeesLocal as &$loc){
       if(isset($masterMap[$loc['EmpNo']])){
         ...
         if(!empty($masterMap[$loc['EmpNo']]['DeptCode'])){ $loc['DeptCode'] = $masterMap[$loc['EmpNo']]['DeptCode']; }
       } else {
...
if ($_SERVER['REQUEST_METHOD']==='POST' && verify_csrf()) {
  ...
  'employee_id' => $selectedEmpId ?: $user['employee_id'] ?? $user['id'],
    ...
    $values = [];
    foreach($insertCols as $c){ $values[] = $fieldMap[$c]; }
...
<div class="container-fluid">
  <?php
    ...
    if(isset($_SESSION['trip_created'])){ $_SESSION['trip_created_displayed']=1; unset($_SESSION['trip_created']); }
  ?>
  <?php if($showSuccess): ?>
    ...
    <script>setTimeout(()=>{var el=document.getElementById('tripSuccessToast'); if(el) el.remove();},5000);</script>
  <?php endif; ?>
  <div class="row">
          ...
          <form method="post" id="tripForm">
            ...
            <div class="mb-4">
              ...
              <?php if($lockedEmployeeId): ?>
                <?php $lr=null; try { $st=$pdo->prepare('SELECT EmpNo, Name, DeptCode FROM employees WHERE id=? LIMIT 1'); $st->execute([$lockedEmployeeId]); $lr=$st->fetch(PDO::FETCH_ASSOC); } catch(Exception $e) {} ?>
                    ...
                    <div class="position-relative">
                      ...
                      <select name="employee_id" id="employeeSelect" class="form-select border-2" size="6" required style="border-radius: 8px; font-size: 0.9rem; background: #fafafa;">
                        ...
                        <?php foreach($employeesLocal as $e): ?>
                          <option data-source="local" data-empno="<?= esc($e['EmpNo']) ?>" data-name="<?= esc($e['Name']) ?>" data-dept="<?= esc($e['DeptCode']) ?>" value="<?= $e['id'] ?>" style="padding: 8px;">
                        ...
                        <?php if($hasJapan): ?>
                          <optgroup label="Japan Employees">
                            <?php foreach($employeesJapan as $e): ?>
                              <option data-source="japan" data-empno="<?= esc($e['EmpNo']) ?>" data-name="<?= esc($e['Name']) ?>" data-dept="<?= esc($e['DeptCode']) ?>" value="<?= $e['id'] ?>" style="padding: 8px;">
                                <span class="fw-medium"><?= esc($e['EmpNo']) ?></span> • <?= esc($e['Name']) ?>
                              </option>
                            <?php endforeach; ?>
                          </optgroup>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(tempInput) tempInput.addEventListener('input', ()=>{ console.log('tempPayment input'); updateConverted();});
  if(currencySel) currencySel.addEventListener('change', ()=>{ console.log('currency change'); updateConverted();});
  // Initial display
